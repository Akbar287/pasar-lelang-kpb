...
<div class="section-header">
    ...
    <div class="section-header-breadcrumb">
        ...
        <div class="breadcrumb-item"><a href="{{ route('konfigurasi.rekening_pusat.show', $rekeningPusat->rekening_pusat_id) }}">Detail</a></div>
        <div class="breadcrumb-item">Ubah</div>
    </div>
</div>
<div class="section-body">
    <h2 class="section-title">Ubah Rekening Pusat</h2>
    <p class="section-lead">
        Ubah data Rekening Pusat anda.
    </p>
    <div class="row justify-content-center">
        <div class="col-md-12">
            @if(session('msg')){!! session('msg') !!} @endif
            <form action="{{ route('konfigurasi.rekening_pusat.update', $rekeningPusat->rekening_pusat_id) }}" method="post" enctype="multipart/form-data">@csrf @method('put')
                <div class="card">
                    <div class="card-header">
                        <h4>{{ __('Ubah Data Rekening Pusat') }}</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row mb-3 justify-content-center">
                                    ...
                                    <div class="col-md-6">
                                        <select class="custom-select @error('bank_id') is-invalid @enderror" name="bank_id" id="bank_id">
                                            @foreach($bank as $ba)
                                            <option {{ $rekeningPusat->bank_id == $ba->bank_id ? 'selected' : '' }} value="{{ $ba->bank_id }}">{{ $ba->nama_bank }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row mb-3 justify-content-center">
                                    ...
                                    <div class="col-md-6">
                                        <select class="custom-select @error('mata_uang') is-invalid @enderror" name="mata_uang" id="mata_uang">
                                            <option {{ $rekeningPusat->mata_uang == 'IDR' ? 'selected' : '' }} value="IDR">IDR</option>
                                            <option {{ $rekeningPusat->mata_uang == 'USD' ? 'selected' : '' }} value="USD">USD</option>
                                        </select>
                                ...
                                <div class="row mb-3 justify-content-center">
                                    ...
                                    <div class="col-md-6">
                                        <select class="custom-select @error('aktif') is-invalid @enderror" name="aktif" id="aktif">
                                            <option {{ $rekeningPusat->aktif == true ? 'selected' : '' }} value="{{ 'true' }}">{{ "Utama" }}</option>
                                            <option {{ $rekeningPusat->aktif == false ? 'selected' : '' }} value="{{ 'false' }}">{{ "Tidak" }}</option>
                                        </select>
                                ...
                                <div class="row mb-3 justify-content-center">
                                    ...
                                    <div class="col-md-6">
                                        <select class="custom-select @error('status') is-invalid @enderror" name="status" id="status">
                                            <option {{ $rekeningPusat->status == true ? 'selected' : '' }} value="{{ 'true' }}">{{ "Aktif" }}</option>
                                            <option {{ $rekeningPusat->status == false ? 'selected' : '' }} value="{{ 'false' }}">{{ "Tidak" }}</option>
                                        </select>
                            ...
                            <div class="col-md-12">
                                <div class="row mb-0">
                                    <div class="col-md-6 offset-md-4 d-flex align-items-end">
                                        <a type="button" href="{{ route('konfigurasi.rekening_pusat.show', $rekeningPusat->rekening_pusat_id) }}" class="btn btn-primary mr-2"><i class="fas fa-arrow-left"></i> Kembali</a>
