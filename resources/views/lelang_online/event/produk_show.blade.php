                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered table-sm">
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>Penjual</th>
                                            <td>{{ !is_null($lelang->kontrak()->first()->informasi_akun()->first()->member()->first()) ? $lelang->kontrak()->first()->informasi_akun()->first()->member()->first()->ktp()->first()->nama : $lelang->kontrak()->first()->informasi_akun()->first()->lembaga()->first()->nama_lembaga }}  (<i class="fas fa fa-star"></i> {{ is_null($lelang->kontrak()->first()->informasi_akun()->first()->rating()->first()) ? '0.0' : $lelang->kontrak()->first()->informasi_akun()->first()->rating()->first()->avg_stars }})</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="col-12 col-sm-12 col-md-4 col-lg-4">
                            <div class="card card-primary">
                                ...
                                <div class="card-footer">
                                    ...
                                    <a href="{{ route('online.event.sesi', $lelang->lelang_id) }}" class="btn btn-danger btn-block"><i class="fas fa-times"></i> Ditutup</a>
                                    ...
                                    <a href="{{ route('online.event.sesi', $lelang->lelang_id) }}" class="btn btn-primary btn-block"><i class="fas fa-clock"></i> Menunggu Sesi Lelang</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row justify-content-between">
                        <div class="col-12 form-custom-buyer d-none">
                            <div class="form-group mt-4">
                                ...
                                <input type="text" required id="no_kode_anggota_penjual" name="no_kode_anggota_penjual" class="form-control" value="{{ is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()) ? '-' : $lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->kode_peserta_lelang }}" placeholder="No. Kode Anggota Penjual" />
                            </div>
                            <div class="form-group">
                                <label for="nama_penjual">Nama Penjual</label>
                                <input type="text" required id="nama_penjual" name="nama_penjual" class="form-control" value="{{ is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()) ? '-' : (is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->member()->first()) ? $lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->lembaga()->first()->nama_lembaga  : $lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->member()->first()->ktp()->first()->nama) }}" placeholder="Nama Penjual" />
                            </div>
                            <div class="form-group">
                                <label for="alamat_penjual">Alamat Penjual</label>
                                <input type="text" required id="alamat_penjual" name="alamat_penjual" class="form-control" value="{{ is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()) ? '-' : (is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->member()->first()) ? ''  : $lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->area_member()->first()->alamat) }}" placeholder="Alamat Penjual" />
                            </div>
                            <div class="form-group">
                                <label for="no_telp_penjual">No. Telepon Penjual</label>
                                <input type="text" required id="no_telp_penjual" name="no_telp_penjual" class="form-control" value="{{ is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()) ? '-' : (is_null($lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->member()->first()) ? '0'  : $lelang->peserta_lelang_berlangsung()->orderBy('waktu', 'desc')->first()->peserta_lelang()->first()->informasi_akun()->first()->no_hp) }}" placeholder="No. Telepon Penjual" />
                            </div>
                        </div>
                    </div>
                </div>
