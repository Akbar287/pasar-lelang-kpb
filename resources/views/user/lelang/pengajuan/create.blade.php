...
<div class="section-body">
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <form action="{{ route('lelang.pengajuan.store') }}" method="post" enctype="multipart/form-data">@csrf
                <div class="card">
                                ...
                                <div class="row mb-3 justify-content-center kontrak">
                                    ...
                                    <div class="col-md-6">
                                        <select class="custom-select @error('kontrak_id') is-invalid @enderror" name="kontrak_id" id="kontrak_id">
                                            ...
                                            <option {{ old('kontrak_id') == $k->kontrak_id ? 'selected' : '' }} value="{{ $k->kontrak_id }}">{{ $k->kontrak_kode }}</option>
                                            @endforeach
                                            @endif
                                        </select>
                                    </div>
                                            ...
                                            <table class="table table-bordered table-striped table-hover">
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td colspan="3" id="kode_komoditas">{{ is_null(old('kontrak_id')) ? $kontrak->first()->komoditas()->first()->komoditas_id : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->komoditas()->first()->komoditas_id }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td><p>Komoditas</p></td>
                                                        <td><p id="isi_komoditas">{{ is_null(old('kontrak_id')) ? $kontrak->first()->komoditas()->first()->nama_komoditas : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->komoditas()->first()->nama_komoditas }}</p></td>
                                                        <td><p>Satuan Transaksi</p></td>
                                                        <td><p id="isi_satuan_transaksi">{{ 'Rp. ' .number_format(is_null(old('kontrak_id')) ? $kontrak->first()->minimum_transaksi : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->minimum_transaksi, 0, ".", ",") }}</p></td>
                                                    </tr>
                                                    <tr>
                                                        <td><p>Jenis Perdagangan</p></td>
                                                        <td><p id="isi_jenis_perdagangan">{{ is_null(old('kontrak_id')) ? $kontrak->first()->jenis_perdagangan()->first()->nama_perdagangan : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->jenis_perdagangan()->first()->nama_perdagangan }}</p></td>
                                                        <td><p>Maks. Transaksi</p></td>
                                                        <td><p id="isi_max_transaksi">{{ 'Rp. ' .number_format(is_null(old('kontrak_id')) ? $kontrak->first()->maksimum_transaksi : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->maksimum_transaksi, 0, ".", ",") }}</p></td>
                                                    </tr>
                                                    <tr>
                                                        <td><p>Mutu</p></td>
                                                        <td><p id="isi_mutu">{{ is_null(old('kontrak_id')) ? $kontrak->first()->mutu()->first()->nama_mutu : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->mutu()->first()->nama_mutu }}</p></td>
                                                        <td><p>UOM</p></td>
                                                        <td><p id="isi_uom">{{ is_null(old('kontrak_id')) ? $kontrak->first()->komoditas()->first()->satuan_ukuran : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->komoditas()->first()->satuan_ukuran }}</p></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="input-group mb-3">
                                            ...
                                            <div class="input-group-append">
                                                <span class="input-group-text kuantitas">{{ count($kontrak) > 0 ? (is_null(old('kontrak_id')) ? $kontrak->first()->komoditas()->first()->satuan_ukuran : $kontrak->where('kontrak_id', old('kontrak_id'))->first()->komoditas()->first()->satuan_ukuran) : 'Kg' }}</span>
                                            </div>
                                        </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row mb-0">
                                    <div class="col-md-6 offset-md-4 d-flex align-items-end">
                                        <a type="button" href="{{ route('lelang.pengajuan') }}" class="btn btn-primary mr-2"><i class="fas fa-arrow-left"></i> Kembali</a>
